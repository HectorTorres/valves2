...
void MainWindow::sensorTimerFunction(){
    ...
    //Determinación para el proceso de inspiración_________________________________
    if(ui->radioButton_assit->isChecked()==true){
    if(indexPress<=4){
   ...
   if((readedPress <= (setPIP/2)) && (slope >= 30)){ //AQUI SERÍA NECESARIO CONSIDERAR QUE CUANDO ES EL CONTROL MEDIANTE VOLUMEN CAMBIAR EL VALOR A LA PRESION MAXIMA ENTRE 2. O UN VALOR MEDIO.
   ...
   //Lectura del flujo y calculo del volumen _________________________________
...
void MainWindow::controlTimerFunction(){
    ...
    if(endLineUpStatus || assistPressDetect){
        ...
        if(assistPressDetect==false){ // Este solo no entra cuando ocurre una inspiración manual, esto debido a que no queremos que los parámetros de tiempo se desconfiguren.
...
void MainWindow::valvesMainControl(uint8_t inspirationValves, uint8_t exalationValves){
    inspirationValves = inspirationValves & 0x0F;
    exalationValves = exalationValves & 0x0F;
...
QString dataOut;
if((exalationValves &  0x08)){ dataOut+="1";} else { dataOut+="0";}
digitalWrite(DATA,(exalationValves &  0x08));
...
if((exalationValves &  0x04)){ dataOut+="1";} else { dataOut+="0";}
digitalWrite(DATA,(exalationValves &  0x04));
...
if((exalationValves &  0x02)){ dataOut+="1";} else { dataOut+="0";}
digitalWrite(DATA,(exalationValves &  0x02));
...
if((exalationValves &  0x01)){ dataOut+="1";} else { dataOut+="0";}
digitalWrite(DATA,(exalationValves &  0x01));
...
if((inspirationValves &  0x01)){ dataOut+="1";} else { dataOut+="0";}
digitalWrite(DATA,(inspirationValves &  0x01));
...
if((inspirationValves &  0x02)){ dataOut+="1";} else { dataOut+="0";}
digitalWrite(DATA,(inspirationValves &  0x02));
...
if((inspirationValves &  0x04)){ dataOut+="1";} else { dataOut+="0";}
digitalWrite(DATA,(inspirationValves &  0x03));
...
if((inspirationValves &  0x08)){ dataOut+="1";} else { dataOut+="0";}
digitalWrite(DATA,(inspirationValves &  0x04));
